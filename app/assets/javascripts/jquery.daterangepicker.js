    ...
    function handleEnd(time)
    {
      ...
      return r;
		...
		function showSelectedInfo(forceValid,silent)
		{
			...
			if (opt.start)
			{
				box.find('.start-day').html('<span class="start-day-val">' + getDateString(new Date(parseInt(opt.start))) + '</span><i class="glyphicon glyphicon-calendar"></i>');
                $('#start-date-input').val(getDateString(new Date(parseInt(opt.start))));
			}
			if (opt.end)
			{
				box.find('.end-day').html('<i class="glyphicon glyphicon-calendar"></i><span class="end-day-val">' + getDateString(new Date(parseInt(opt.end)))+'</span>');
                $('#end-date-input').val(getDateString(new Date(parseInt(opt.end))));
			}
		...
		function createDom()
		{
            ...
            {
                html += '<div class="shortcuts"><p>'+lang('shortcuts')+'</p></div>';
                ...
                if (data)
                {
                    if (data['prev-days'] && data['prev-days'].length > 0)
                    {
                        ...
                        for(var i=0;i<data['prev-days'].length; i++)
                        {
                            ...
                            name = name + ' ';
                            name += (data['prev-days'][i] > 3) ? lang('days') : lang('day');
                            html += ' <a href="javascript:;" shortcut="day,-'+data['prev-days'][i]+'">'+ lang('past') + ' ' + name +'</a>';
                        }
                        html+='</span>';
                    }
			...
			html += '<div class="drp_top-bar">\
					<div class="normal-top">\
						<span style="color:#333">'+' </span> <b class="start-day">...</b>'
			if ( ! opt.singleDate ) {
				html += ' <span class="separator-day"></span> <b class="end-day">...</b>'
			}
			html += '</div>\
				...
				+'<div class="month1-wrap"><table class="month1" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;"><span class="prev">&lt;</span></th><th colspan="5" class="month-name">January, 2011</th><th style="width:27px;">' + (opt.singleDate || !opt.stickyMonths ? '<span class="next">&gt;</span>': '') + '</th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table></div>'
			if ( ! opt.singleDate ) {
				html += '<div class="gap">'+getGapHTML()+'</div>'
					+'<div class="month2-wrap"><table class="month2" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;">' + (!opt.stickyMonths ? '<span class="prev">&lt;</span>': '') + '</th><th colspan="5" class="month-name">January, 2011</th><th style="width:27px;"><span class="next">&gt;</span></th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table></div>'
			}
				//+'</div>'
